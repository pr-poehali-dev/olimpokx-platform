...
const exams = [
  { id: 1, title: "Охрана труда 2026", course: "Охрана труда", date: "15 апр 2026", duration: "60 мин", questions: 40, attempts: 2, maxAttempts: 3, status: "scheduled", score: null, passing: 80 },
  { id: 2, title: "Итоговый: Управление проектами", course: "Управление проектами", date: "28 апр 2026", duration: "90 мин", questions: 60, attempts: 0, maxAttempts: 2, status: "available", score: null, passing: 75 },
          ...
          {selectedExam ? (
            <div className="space-y-4">
              ...
              {(selectedExam.status === "available") && (
                <button className="w-full bg-primary text-white py-2.5 rounded text-sm font-semibold hover:bg-primary/80 transition-colors">
                  Начать экзамен
                </button>
              )}
              {selectedExam.status === "passed" && (
                <button className="w-full bg-muted text-foreground py-2.5 rounded text-sm font-medium border border-border hover:bg-secondary transition-colors">
                  Скачать сертификат
                </button>
              )}
            </div>
